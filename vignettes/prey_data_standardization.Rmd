...
-   created a modified version of the tab *Density by location, date...*
    to make it easier to load into R
...
-   removed the total prey density and left the prey density
...
-   Match names of locations to the `stanislaus_zoop` table and merge
    them
...
-   Data is currently located on EDI repository and downloaded directly
    into R
...
-   extracted temperature and dissolved oxygen from zooplankton data.
    `site` should link the two datasets.
...
-   downloaded locations data from EDI and manually assigned each
    habitat type based on best professional judgement
